...
const schema: VbenFormSchema[] = [
  ...
  },
  {
    ...
    fieldName: 'region',
    label: '访问区域',
    help: '腾讯云COS的服务区域，例如：ap-beijing',
    rules: z.string().min(1, '访问区域不能为空'),
  },
  {
    component: 'Input',
    fieldName: 'secretId',
  ...
  },
  {
    ...
    rules: z.string().min(1, '密钥Key不能为空'),
  },
  {
    component: 'Input',
  ...
  },
  {
    ...
    label: '文件访问域名',
    help: '文件访问的完整域名地址，例如：https://your-bucket-1234567890.cos.ap-beijing.myqcloud.com',
    ...
    if (isOpen) {
      ...
      nextTick(() => {
        ...
        if (data && data.id) {
          ...
          // 如果是编辑模式且有腾讯云COS配置数据
          if (
            data.storageType === 'tencent_cos' &&
            data.tencentCosStorageConfigVo
          ) {
            Object.assign(formValues, {
              region: data.tencentCosStorageConfigVo.region,
              secretId: data.tencentCosStorageConfigVo.secretId,
              secretKey: data.tencentCosStorageConfigVo.secretKey,
              bucketName: data.tencentCosStorageConfigVo.bucketName,
              fileDomain: data.tencentCosStorageConfigVo.fileDomain,
              realDelete: data.tencentCosStorageConfigVo.realDelete,
            });
          }
  ...
  if (valid) {
    ...
    try {
      if (formData.value?.id) {
        ...
        await addTencentCosStorageConfig(createData);
      }
...
const getDrawerTitle = computed(() => {
  ...
  return isEditMode ? '修改腾讯云COS存储配置' : '新增腾讯云COS存储配置';
});
    ...
    <div class="mx-4">
      <div class="mb-6">
        <div class="mb-2 flex items-center space-x-2">
          <div
            class="flex h-8 w-8 items-center justify-center rounded bg-blue-500"
          >
            <span class="text-sm font-bold text-white">腾</span>
          </div>
        ...
        <p class="text-sm text-gray-500">
          配置腾讯云对象存储服务的连接参数，提供安全稳定、海量、便宜的云端存储服务
        </p>
      </div>
